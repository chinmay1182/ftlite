...
def init(registry):
    ...
    if not os.path.exists(registry):
        ...
        with open(registry, "w") as f:
            json.dump(
                {"entities": {}, "feature_views": {}, "on_demand_feature_views": {}},
                f,
                indent=2,
            )
...
@click.option(
    "--online-db",
    default=".ftlite/online_store.db",
    help="Path to online store database file.",
)
@click.option(
    "--start",
    required=True,
    help="Start time in ISO 8601 format (e.g. 2026-06-01T00:00:00).",
)
@click.option(
    "--end",
    required=True,
    help="End time in ISO 8601 format (e.g. 2026-07-01T00:00:00).",
)
@click.option(
    "--views",
    help="Comma-separated list of feature views to materialize. If omitted, all views are materialized.",
)
...
def list(registry):
    ...
    if not os.path.exists(registry):
        click.echo(
            f"Registry file not found at {registry}. Run 'ftlite init' first.", err=True
        )
        return
    ...
    for ent in entities:
        click.echo(
            f"  {ent.name} (join_key: {ent.join_key}, value_type: {ent.value_type})"
        )
