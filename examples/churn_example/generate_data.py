...
def generate_churn_dataset(dest_dir: str):
    ...
    signup_dates = [
        datetime(2026, 1, 1) + timedelta(days=int(np.random.randint(0, 100)))
        for _ in range(num_customers)
    ]
    plan_types = np.random.choice(
        ["Basic", "Premium", "Enterprise"], size=num_customers, p=[0.5, 0.3, 0.2]
    )
    ...
    for customer_id in customer_ids:
        ...
        for week in range(4):
            timestamp = current_date + timedelta(
                days=int(np.random.randint(0, 5)), hours=int(np.random.randint(0, 24))
            )

            # Simulate features
            usage_minutes = float(
                np.random.normal(loc=120.0, scale=30.0)
                + (100.0 if plan_types[customer_id - 10001] == "Premium" else 0)
            )
            support_calls = int(
                np.random.poisson(
                    lam=0.5 if plan_types[customer_id - 10001] == "Basic" else 0.1
                )
            )
